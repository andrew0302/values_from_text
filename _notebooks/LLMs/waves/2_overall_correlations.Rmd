...
wave_2_0na_mean_dfs <- lapply(wave_2_0na_dfs[1:2], function(df){
...
```{r}
# # add factor scores to overall dfs
# 
# dfs <- imap(overall_dfs[['lyrics1']], ~ rename(.x, !!.y := rating))
# 
# df <- reduce(dfs, full_join, by = c("item_ID", "value"))
#   
# df <- df %>% select(item_ID, value, `gemma2:9b`, phi4, `qwen2.5:7b`, participant_mean, participant_mean_0na)
# 
# fa <- fa(df%>%select(`gemma2:9b`, phi4, `qwen2.5:7b`), nfactors = 1, scores = "regression")
# df$fa <- fa$scores %>% as.numeric()
# 
# df <- df %>% select(item_ID, value, fa, participant_mean, participant_mean_0na) %>%
#   rename(rating = fa)
# 
# overall_dfs[['lyrics1']][['fa']] <- df
```
...
```{r}
# fa <- overall_dfs[['lyrics1']][['fa']] %>% 
#   select(item_ID, value, rating) %>% 
#   pivot_wider(names_from = value, values_from = rating)
# 
# llm_mean_dfs[['lyrics']][['fa']] <- fa
