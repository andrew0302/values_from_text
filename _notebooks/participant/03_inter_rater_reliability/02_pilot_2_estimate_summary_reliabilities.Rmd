---
title: "pilot: estimate summary reliabilities"
---
...
estimate_overall_icc <- function(df){
...
saveRDS(pilot_2_likert_summary_dfs, here(file_path, "pilot_2_likert_summary_dfs.RDS"))
```

```{r}
# select relevant columns and pivot a results dataframe
format_ICC_1_df <- function(df){
  ...
  colnames(df) <- c("value", 
                    "ICC1", "ICC1k", 
                    "ICC1_lower", "ICC1k_lower", 
                    "ICC1_upper", "ICC1k_upper")
...
```{r}
# estimate ICC and ICC2k per value for a given dataset
make_overall_icc_1_df <- function(df){
  ...
  # select relevant ICC output
  icc_df <- icc_df$results %>% filter(type =="ICC1" | type =="ICC1k")
  icc_df$value <- value
  return(icc_df)
})
...
```{r}
pilot_2_lyrics_icc_1_df <- lapply(pilot_2_lyrics_subsample_list, make_overall_icc_1_df) %>% 
  bind_rows(.id = "subset")
pilot_2_speeches1_icc_1_df <- lapply(pilot_2_speeches1_subsample_list, make_overall_icc_1_df) %>%
  bind_rows(.id = "subset")
pilot_2_speeches2_icc_1_df <- lapply(pilot_2_speeches2_subsample_list, make_overall_icc_1_df) %>% 
  bind_rows(.id = "subset")

pilot_2_likert_summary_icc1_dfs <- list(lyrics = pilot_2_lyrics_icc_1_df, 
                                   speeches1 = pilot_2_speeches1_icc_1_df, 
                                   speeches2 = pilot_2_speeches2_icc_1_df)

saveRDS(pilot_2_likert_summary_icc1_dfs, here(file_path, "pilot_2_likert_summary_icc1_dfs.RDS"))
```
