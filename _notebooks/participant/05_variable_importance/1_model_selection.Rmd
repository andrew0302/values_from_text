...
clmm and glmer models fail to converge for both lyrics and speeches

```{r}
spec_lyrics    <- as.formula("rating ~ value + Ethnicity * genre + (1|participant_ID) + (1|item_ID)")
spec_speeches1  <- as.formula("rating ~ value + Ethnicity * party + (1|participant_ID) + (1|item_ID)")
spec_speeches2  <- as.formula("rating ~ value + spectrum * party + (1|participant_ID) + (1|item_ID)")
```
...
wave_2_speeches_ethnicity_mods <- list(
...
saveRDS(wave_2_speeches_ethnicity_mods, here("_data", "_intermediary_data", "models", "wave_2_speeches_mods.RDS"))
```
