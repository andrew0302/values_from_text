...
subjective_df <- plot_df |> 
  select(participant_ID, subjective, study) |> 
...
# Group medians (lyrics vs speeches)
group_medians <- subjective_df |>
  group_by(study) |>
  summarise(
    median_subjective = median(subjective, na.rm = TRUE),
    .groups = "drop"
  )

# Wilcoxon rank–sum test
wtest <- wilcox.test(subjective ~ study, data = subjective_df, exact = FALSE)
W_stat <- wtest$statistic
p_val  <- wtest$p.value
...
# Combined LaTeX sentence
latex_sentence <- sprintf(
  "Participants rated the task as moderately subjective overall (median = %s), with %.1f\\%% of responses falling on the subjective side of the scale. Subjectivity ratings differed by text type: lyrics had a median rating of %s, while speeches had a median of %s. A Wilcoxon rank--sum test confirmed this difference ($W = %s$, %s).",
  overall_stats$median_value,
  overall_stats$pct_over_4,
  median_lyrics,
  median_speeches,
  W_stat,
  p_clean
