...
plot_grid(
  ...
  SSVS_df %>% select(all_of(values)) %>% cor() %>% 
                           mds_plot(., "Our Participants' SSVS Scores"), 
  ncol = 2
)

ggsave(here("images", "wave_2_participant_MDS.png"))
```
...
plot_grid(
  ...
  bind_rows(mean_dfs[[2]][[3]], mean_dfs[[2]][[4]]) %>% select(all_of(values)) %>% cor(use = "complete.obs") %>% mds_plot(., "Speeches"), ncol = 3
)

ggsave(here("images", "wave_2_lyrics_speeches_MDS.png"), height = 5, width = 9)
```
