...
p1 <- lyrics_df |>
  ...
  pivot_wider(names_from = value, values_from = rating) |>
...
p2 <- speeches_df |>
  ...
  pivot_wider(names_from = value, values_from = rating) |>
  select(-item_ID, -participant_ID) |>
  plot_miss() + xlab("") + ylab("% NA")

na_plot <- (p1 | p2) + plot_annotation(tag_levels = "A")
na_plot

#ggsave(here("images", "main_missingness_by_text.pdf"))
ggsave(here("images", "main_missingness_by_text.pdf"), width = 8, height = 4)
```
